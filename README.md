...
13. 增强分屏编辑体验，代码区和预览区支持按滚动比例同步。
14. 修复公开文档访问链路，HTML 和 Markdown 公开页都会直出渲染，不依赖登录态界面。
---
...
- **分屏同步滚动**：分屏模式下，代码编辑区和 HTML / Markdown 预览区会按滚动比例互相同步，长文档编辑时更容易定位内容。
...
公开访问页会直接展示文档内容：HTML 入口使用沙箱 iframe 打开，Markdown 入口直接渲染成阅读页面。未登录访问公开文档时不会显示编辑器、文档树或登录弹窗。
